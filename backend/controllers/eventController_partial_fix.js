...
const eventController = {
  // Get all events for logged-in organizer with real application statistics
  getOrganizerEvents: async (req, res) => {
    try {
      ...
      const query = `
        ...
        GROUP BY e.id, e.title, e.description, e.event_type, e.event_date, e.start_time, e.end_time,
                 ...
                 e.contact_info, e.is_public, e.status, e.organizer_id, e.created_at, e.updated_at
      ...
      // Format JSON fields and add real application statistics
      const formattedEvents = result.data.map(event => ({
        ...
        // Use real application statistics for compatibility with existing frontend
        total_bookings: parseInt(event.total_applications) || 0,
        confirmed_bookings: parseInt(event.approved_applications) || 0,
        pending_bookings: parseInt(event.pending_applications) || 0,
        rejected_bookings: parseInt(event.rejected_applications) || 0,
        // Also keep the proper field names
        total_applications: parseInt(event.total_applications) || 0,
        pending_applications: parseInt(event.pending_applications) || 0,
        approved_applications: parseInt(event.approved_applications) || 0,
        rejected_applications: parseInt(event.rejected_applications) || 0
      }));
  ...
  browseEvents: async (req, res) => {
    try {
      ...
      console.error('âŒ Browse events error:', error);
      console.error('âŒ Error stack:', error.stack);
      console.error('âŒ Request query:', req.query);
      res.status(500).json({
        success: false,
